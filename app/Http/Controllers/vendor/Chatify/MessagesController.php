...
class MessagesController extends Controller
{
    ...
    public function download($fileName)
    {
        if (Chatify::storage()->exists(config('chatify.attachments.folder') . '/' . $fileName)) {
            ...
            return abort(404, "Извините, файл не существует на нашем сервере или, возможно, был удален!");
        }
    }
    ...
    public function fetch(Request $request)
    {
        ...
        if ($totalMessages < 1) {
            $response['messages'] ='<p class="message-hint center-el"><span>Скажите «привет» и начните обмен сообщениями</span></p>';
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        // send the response
        return Response::json([
            'shared' => count($shared) > 0 ? $sharedPhotos : '<p class="message-hint"><span>Ещё нет общих файлов</span></p>',
        ], 200);
    }
